...
`content` accepts a value of any type and stringifies it, exactly as `{expr}` interpolation does. Mixing a non-string into a `+` expression is still a runtime error: convert through interpolation instead.
