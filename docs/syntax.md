...
- The `default` accepts any expression - a literal, a previously declared variable, or a computed expression like `lower(trim(project_name))`. The expression is evaluated against the live environment when the user skips the prompt. The default's type must match the declared ask type; mismatches are rejected at parse time for literals and at validate time for identifier references whose type can be inferred.
- `options` restricts valid answers to a fixed set, presented as a numbered menu. The user may type either the literal value or its 1-based number. When both `options` and `default` are present, the default value must equal one of the listed options - otherwise the validator rejects the program.
